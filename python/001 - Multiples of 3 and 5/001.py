...
We need to consider if there is any Double Counting going on in our logic, which is revealed to be the case when we consider numbers that a multiples of both 3 and 5, a.k.a multiples of 15 (3*5)
Therefore, when we come across a multiples of 15, we need to determine what we will do with it.
...
Secondly, we can approach this problem using some simple mathematical forumla for the sum of all integers up to n, this being (1/2)*(n)*(n+1). 
...
To be honest, either option here is probably fine considering the question itself is only asking for multiples up to 1000, but I'm going to write this program to just take any int n given, so might as well optimise to the best of my ability.
"""
